...
public record BorrowRecordResponse(Long id, Long bookCopyId, Long borrowerPersonId, LocalDate borrowedDate,
		LocalDate dueDate, LocalDate returnedDate, String status, String borrowerName, String bookTitle,
	...
	public static BorrowRecordResponse from(BorrowRecord borrowRecord) {
		return new BorrowRecordResponse(borrowRecord.getId(), borrowRecord.getBookCopy().getId(),
				...
				borrowRecord.getBorrower().getFirstName() + " " + borrowRecord.getBorrower().getLastName(),
				borrowRecord.getBookCopy().getBook().getTitle(), borrowRecord.getBookCopy().getAccessionNumber());
	}
}
