...
- A long-running `./gradlew bootRun` dev backend can predate the code you're testing — if a newly-added endpoint 401s or 404s unexpectedly, don't assume it's an auth/routing bug before checking the process's uptime and Flyway log for whether the latest migrations actually applied; restart it if stale. 401 alone isn't a reliable "route doesn't exist" signal either — auth interception happens before route matching.
...
### Milestone 5 — Inventory, completed (2026-08-28, #53/#54/#55)

Two feature builds plus one scoping decision, no schema migration needed (both features compute off data Milestone 4 already put in place). `InventoryService.getLowStockItems(Campus)` (#53) computes balances for every active item with a reorder level set via three grouped-sum queries (one per stock table, keyed by item id) rather than per-item lookups, so cost stays fixed regardless of catalog size; filters to balance <= reorderLevel. New `GET /api/v1/inventory/items/low-stock?campusId=`; `ItemsPanel.tsx` gained a campus picker, a "Low stock only" filter, and a warning-colored chip per row. Covered by `LowStockTest.java`. #54's unified stock ledger was built client-side per the ticket's own suggested option — `ItemDetailPage.tsx`'s three separate entry/issue/adjustment tables became one merged, date-sorted table (signed quantity, type chip) computed via `useMemo` from the same three existing list calls; no backend endpoint added. #55 (Warehouse/Rack sub-campus locations) was a scoping-decision ticket, same pattern as the Subject/Curriculum ticket in Phase E — resolved by deferral: filed **#191** (StorageLocation entity under Campus, optional FK from the three stock tables) explicitly gated on a concrete workflow needing it, not scheduled to a milestone; Warehouse/Rack dropped from M5's actual scope, nothing built against it.

### Milestone 6 — Purchase, completed (2026-08-28, #56/#57/#58/#59)

The actual procurement build `erp-system-plan.md`'s "don't build a warehouse/procurement system unless a school actually needs it" warning was about - re-confirmed with the user before starting (they chose "build it all"), unlike M4/M5 which deliberately stayed master-data-only. New `com.operion.purchase` package (V47-V49), depending on `com.operion.inventory`/`InventoryService` but never the reverse - Inventory has zero knowledge of Purchase Orders. `PurchaseOrder` runs DRAFT -> SUBMITTED -> APPROVED -> (PARTIALLY_RECEIVED ->) RECEIVED, with CANCELLED reachable only before any receiving starts (#56); `PurchaseOrderLine` references its parent by FK only, no owning-side collection, same convention as StockEntry/StockIssue/StockAdjustment referencing Item/Campus. Receiving (#57) calls `InventoryService.recordEntry` per line (reusing the existing balance/ledger logic rather than duplicating it) and the PO's status is derived from whether every line is fully received. Returns (#58) are the interesting design call: `PurchaseReturn` is a dedicated insert-only audit trail back to the PO/supplier (what a generic StockAdjustment row can't provide), but the actual balance decrement is a `StockAdjustment(SUPPLIER_RETURN)` written alongside it via `InventoryService.recordAdjustment` - reusing the existing negative-balance guard rather than inventing a fourth balance-affecting ledger table that `getBalance` would need to know about (that would have meant Inventory depending on Purchase, backwards). `SUPPLIER_RETURN` was added to `StockAdjustmentReason` for this - not exposed in the manual adjustment dialog, set only by `PurchaseOrderService`. Frontend (#59): `PurchaseOrdersPanel`/`PurchaseOrderDetailPage` under a new "Purchase" nav item (`PURCHASE_VIEW`), following the Item/ItemDetailPage list+find convention but with a real `GET /{id}` (line-item detail pages are too heavy for the list-and-filter trick). Covered by `PurchaseOrderLifecycleTest.java` (8 tests: status transitions, over-receive/over-return rejection, balance effects) plus a full UI-driven smoke test (create -> submit -> approve -> receive -> return) through a headless browser before considering it done - see the "Running/verifying changes" note below for how, since no project-specific run skill exists yet for this app.

**Running/verifying changes** (no project skill exists for this yet - a candidate for `/run-skill-generator` if this recurs often): `./gradlew bootRun` for the backend (`http://localhost:8080`), `cd web && npm run dev` for the frontend (`http://localhost:5173`, Vite HMR picks up edits live). **The stale-backend gotcha above applies** - if the dev backend was already running before a session's edits, kill and restart it (`lsof -ti:8080 -sTCP:LISTEN | xargs kill`, wait for the port to free, then `nohup ./gradlew bootRun` in the background) before trusting any manual verification against it; check the Flyway log lines for "Migrating schema" to confirm new migrations actually applied. Browser-driven verification used Playwright directly (`chromium-cli` wasn't available in this environment) - a global install happened to exist at `/private/tmp/gam_pw/node_modules/playwright`, loaded via its absolute path rather than a project dependency. Provisioning a fresh org via `POST /api/v1/organisations` is cheap and avoids guessing at credentials for one of the many pre-existing dev-DB orgs (`operion.organisations` has accumulated a lot of `verify-*`/test slugs from past sessions - harmless, never cleaned up).

### Examinations & Results and Fees & Finance gap-closing (2026-09-13, GitHub milestones 17 & 18)

Both closed end to end - not new modules, gap-closing passes against the competitive analysis below, tracked as GitHub milestones per already-built module rather than the numbered Milestone scheme above (that scheme's own Milestones 1-9 plus Purchase/Sales are all long since built - Sales in particular is live in the nav despite the stale note this section used to carry).

**Examinations & Results** (#134-#139, #243): almost all already implemented in a prior session's single large commit (`MarksEntryRegister` DRAFT->SUBMITTED->APPROVED workflow with its own `MARKS_SUBMIT`/`MARKS_APPROVE` permissions, `ExamSchedule` class-vs-section scoping, pass/fail via a configurable `PassFailStrategy` on `ExaminationSettings`, subject/class rank with standard-competition tie-breaking gated by an org-level `rankingEnabled` toggle, publish/stale/republish/supersede lifecycle on `ReportCard` for post-publish corrections gated by a distinct `MARKS_CORRECT_AFTER_PUBLISH` permission) - this session's work was auditing that it was actually correct and filling the missing test coverage (schedule-conflict rejection, the two non-default pass-fail strategies, rank tie-breaking). #243 (printable/branded report card) stays open, blocked on #142/#31 for real letterhead/PDF infra - the render-endpoint-plus-print-view is the maximal in-scope contribution already done.

**Fees & Finance** (#130-#133, #203, #237): genuinely new work this session. `Adjustment`/`Waiver` entities (V89) mirror `Refund`'s shape exactly (insert-only, reference the `Invoice`, require reason+approvedBy, own permission each - `FEE_ADJUSTMENT_MANAGE`/`FEE_WAIVER_APPROVE`) and fold additively into `Invoice.totalAmount` via `applyAdjustment()`/`applyWaiver()`, guarded so an adjustment can't drop the total below `amountPaid` and a waiver can't exceed the outstanding balance. **Deliberate decisions, not gaps**: no late-fee/penalty mechanism at all (#130 - explicit policy call, closed as not-planned) and Credit/wallet ledger deferred with no tracking issue (#131 - bigger feature, own design pass warranted, revisit only when a concrete workflow needs it). New `FeeReminderService` (cross-cutting like `dashboard`, depends on both `com.operion.parent` and `com.operion.communication`) powers the demand view's "Send reminders" action by routing through the existing `CommunicationService`/`NotificationRecipient` pipeline - genuine EMAIL/SMS dispatch already existed (`NotificationDispatchWorker`, real Resend/Brevo calls), so this was pure integration, not a second notification path; recipients are guardians who've opted into `StudentGuardian.canReceiveCommunication`. Frontend: `FeesPage` split into `FeesSetupPage`/`FeeCollectionPage` (#203, two separate route components, not a route-branching single component); `StudentFeesPanel` gained a payment-history table with a receipt-number confirmation dialog (#133) and Refund/Mark-bounced/Adjust/Waive row actions (#132/#131, permission-gated via `<Can>`); new `FeeDemandPage` (`/fees/demand`, #237) for the overdue-invoices view + bulk reminders.

**Local dev gotcha hit this session**: this machine's MySQL root@localhost rejected the README's documented blank password - needed `DB_PASSWORD=mysql` to boot the backend. Not a codebase issue, just a local environment mismatch worth checking first if `bootRun` fails with `Access denied for user 'root'@'localhost'`.

---

**Not yet decided** (surface options before picking): backfilling Academic Foundation's test gap vs. `FEE_DISCOUNT_APPROVE` vs. a production build/deploy story vs. per-button RBAC retrofit vs. rolling the Fees module's deeper design-system treatment out to the remaining modules. Check the GitHub milestones list directly for what's queued next - Examinations & Results and Fees & Finance (this session's two) are both fully closed now.

**Competitive gap analysis (edumerge/schools24.in)**: sessions compared Operion feature-by-feature against edumerge.com and schools24.in's School ERP offerings (published as Artifacts, conclusions only — not fetchable from here). Highest-leverage finding, now half-closed: parents/students had no login path at all, which blocked most of the P0/P1 backlog. Parent Portal + Payment Gateway close the identity half. Still not started: the `/api/v1/me/**` read API (the actual payoff), a timetable/period structure (Attendance is capped at daily-only because of this), and bulk data import. The first org-admin dashboard shipped 2026-08-27 (Organisation Structure/Dashboard section above); Examinations & Results and Fees & Finance gap-closing shipped 2026-09-13 (section above).
