...
**Not yet decided** (surface options before picking): backfilling Academic Foundation's test gap vs. `FEE_DISCOUNT_APPROVE` vs. a production build/deploy story vs. per-button RBAC retrofit vs. rolling the Fees module's deeper design-system treatment out to the remaining nine modules. Milestone 5 (Inventory, #53–#55) is now fully closed out (pushed + issues/milestone closed 2026-08-28). Milestones 6 (Purchase — PO/GoodsReceipt/Return) and 7 (Sales) are the likely next candidates; [[project-milestone4-paused-procurement-scope]] flags that both specifically carry the same procurement-scope tension M4 was re-confirmed against — re-surface it before starting either, same as was done for M4 and (via #55's deferral) M5.
