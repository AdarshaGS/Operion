...
**Not yet decided** (surface options before picking): backfilling Academic Foundation's test gap vs. `FEE_DISCOUNT_APPROVE` vs. a production build/deploy story vs. per-button RBAC retrofit vs. rolling the Fees module's deeper design-system treatment out to the remaining nine modules. Milestone 6 (Purchase, #56–#59) is now fully closed out (pushed + issues/milestone closed 2026-08-28). Milestone 7 (Sales) is the likely next candidate; [[project-milestone4-paused-procurement-scope]] flags that it still carries the same procurement-scope tension M4/M6 were re-confirmed against - re-surface it before starting, same as was done for M4 and M6.
